...
public class IapController : MonoBehaviour, IStoreListener
{
    ...
    public void BuyProductNotInapp(TypePackIAP typePack)
    {
        ...
        else if (item.typeBuy == TypeBuy.Coin)
        {
            ...
            {
                this.PostEvent(EventID.NOT_ENOUGH_COIN_TO_BUY, typePack.ToString());
                //Confirm box
    ...
    private void ProcessEventSuccess(Product product)
    {
        ...
        {
            var pack = inappDatabase.lstPacksInapp[i];
            //purchasedPacks.Add(pack.type);
            //GameController.Instance.useProfile.PurchasedPack = JsonConvert.SerializeObject(purchasedPacks);
            if (pack != null)
            {
