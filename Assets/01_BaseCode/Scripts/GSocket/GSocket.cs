...
public class GSocket : SingletonClass<GSocket>, IService
{
    ...
    {
//#if UNITY_EDITOR
//        OnLoginGoogle($"GoogleID_{GUtils.GetDeviceId()}", "ANDROID PHONE", "https://lh3.googleusercontent.com/Hb_qe3TdicCZ_swaic5tu3unuZzD8yx-iBOnIW6zq6ORyWE56OpB8KPj1ph6dWRfIwE=w144-h144-n", "12345678910");
//        return;
//#endif
//        WaitingBox.Setup().ShowWaiting(15);
...
//            if (task.IsFaulted)
//            {
//                ConfirmBox.Setup().AddMessageYes(Localization.Get("tit_error"), Localization.Get("lb_login_fail"));
...
//                        Debug.LogError(error.Message + "| " + error.Status);
//                    }
//                }
//            }
//            else
//            {
//                var ID = task.Result.UserId;
//                var DisplayName = task.Result.DisplayName;
//                var ImageUrl = task.Result.ImageUrl.AbsoluteUri;
//                var authorCode = task.Result.AuthCode;

//                OnLoginGoogle(ID, DisplayName, ImageUrl, authorCode);
//            }
//        });

    }

    private void OnLoginGoogle(string ID, string DisplayName, string ImageUrl, string authCode)
    {
        UserDataServices.Instance.AllowUpload(false, "GoogleLoginRequest");
       
        Debug.LogError("Unldo Login");
        GameServices.Instance.LoadMyAvatar();
#if !UNITY_EDITOR
            //    GoogleSignIn.DefaultInstance.SignOut();
#endif
     //  EventDispatcher.EventDispatcher.Instance.PostEvent(EventID.ON_LOGIN_GG_OR_FB_COMPLETE);
        WaitingBox.Setup().HideWaiting();
        UserDataServices.Instance.AllowUpload(true, "RocketServices_LoginSequence_LinkGoogleRequest_Error");
        UseProfile.IsLoggedIn = true;

        UseProfile.AuthCodeGG = authCode;
        UseProfile.AvatarLink = ImageUrl;
        UseProfile.NamePlayer = DisplayName;
    ...
    private void HandleConfirmDataLocal_GG()
    {
        WaitingBox.Setup().HideWaiting();
        EventDispatcher.EventDispatcher.Instance.PostEvent(EventID.ON_LOGIN_GG_OR_FB_COMPLETE);
    }
    private void HandleConfirmDataSever_GG()
    {
        WaitingBox.Setup().ShowWaiting();
