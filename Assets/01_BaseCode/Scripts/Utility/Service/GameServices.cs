...
public class GameServices : SingletonClass<GameServices>, IService
{
    ...
    public void Init()
    {
       ...
       // InitFacebook();
        ...
        //fbManager.GetAllFriendsSuccessAction += GenerateFriendList;
        //fbManager.LogoutObservable()
        //         .CatchIgnore()
        //         .Subscribe(_ =>
        //         {
        //             MyAvatar.Value = defaultAvatar;
        //         });
        //fbManager.FirstInit();
        //LoadMyAvatar(UseProfile.FacebookID);
    }
    ...
    public void LoadMyAvatar(string facebookId = "")
    {
        if (RemoteConfigController.GetBoolConfig("load_avatar_by_id", true))
        {
            if (!string.IsNullOrEmpty(facebookId))
            {
                var _url = Uri.EscapeUriString(string.Format("https://graph.facebook.com/{0}/picture?type=large", facebookId));
                UseProfile.AvatarLink = _url;
            }
        }

        string avatarURL = UseProfile.AvatarLink;
        if (!string.IsNullOrEmpty(avatarURL))
            Context.DownloadOrCache(avatarURL, true)
                .CatchIgnore()
                .Subscribe(_ =>
                {
                    MyAvatar.Value = _;
                    _isLoadAvatarDone = true;
                    EventDispatcher.EventDispatcher.Instance.PostEvent(EventID.LOAD_AVATAR_SUCCESS);
                });

    }

    public void LoadMyFlag()
    {
        if (_myflag != null) return;
        var url = Uri.EscapeUriString(string.Format(RemoteConfigController.GetStringConfig("flag_url", Config.FLAG_API), Context.CountryCode));
        Context.DownloadOrCache(url, isLoadAvatar: false)
            .CatchIgnore()
            .Subscribe(_ =>
            {
                if (_ != null)
